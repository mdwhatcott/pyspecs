...
class _This(object):
  ...
  def be_greater_than(self, lesser):
    self._assert(
      ...
      report=lambda: (self._expect + GREATER_THAN).format(self._value, lesser)
    )

  def be_less_than(self, greater):
    self._assert(
      action=lambda: self._value < greater,
      report=lambda: (self._expect + LESS_THAN).format(self._value, greater)
    )
  ...
  def be_between(self, first, last):
    self._assert(
      ...
      report=lambda: (self._expect + BETWEEN).format(self._value, first, last)
    )
